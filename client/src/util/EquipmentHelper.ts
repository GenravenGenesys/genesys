import type {Quality, Armor, Weapon, Equipment, EquipmentQuality} from "../api/model";
...
export const renderQualities = (equipment: Equipment): string => {
    ...
    if (!(equipment.qualities === undefined || equipment.qualities.length > 0)) {
        ...
        const qualityList = equipment.qualities.sort((a: EquipmentQuality, b: EquipmentQuality) => a.name.localeCompare(b.name))
