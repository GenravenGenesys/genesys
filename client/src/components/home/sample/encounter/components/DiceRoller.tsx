...
interface DiceRollerProps {
    ...
    onClose: () => void;
    onRollComplete: (
        result: DiceResult | { success: number; advantage: number }
    ) => void;
}
...
export const DiceRoller: React.FC<DiceRollerProps> = ({
    ...
    const handleAutoRoll = () => {
        if (rollType === "initiative") {
            const rollSuccess = Math.floor(Math.random() * 3) + 1;
    ...
    const handleManualUpdate = (field: keyof DiceResult, value: number) => {
        setResult((prev) => ({
            ...prev,
    ...
    const netSuccess =
        result.success + result.triumph - result.failure - result.despair;
    ...
    return (
        <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
            <DialogTitle>
                Roll {rollType === "initiative" ? "Initiative" : "Dice"}:{" "}
                {participantName}
            </DialogTitle>
                ...
                <Box sx={{ mb: 3 }}>
                    {rollType === "initiative" ? (
                        <Alert severity="info" sx={{ mb: 2 }}>
                            Roll Cool or Vigilance for initiative
                        </Alert>
                    ) : (
                        <Alert severity="info" sx={{ mb: 2 }}>
