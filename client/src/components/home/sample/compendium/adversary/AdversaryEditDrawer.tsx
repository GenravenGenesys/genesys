    ...
    return (
        <Drawer anchor="right" open={open} onClose={onClose} PaperProps={{ sx: { width: 500, p: 4, bgcolor: '#050c14' } }}>
            ...
            <Stack spacing={3}>
                ...
                <Grid container spacing={1}>
                    {Object.keys(formData.characteristics).map(char => (
                        <Grid size={4} key={char}>
                            <TextField label={char} type="number" size="small" value={formData.characteristics[char]} onChange={(e) => updateChar(char, e.target.value)} />
                        </Grid>
                    ))}
                </Grid>
                ...
                <Grid container spacing={1}>
                    <Grid size={4}><TextField label="Combat" type="number" value={formData.ratings.combat} onChange={(e) => updateRating('combat', e.target.value)} /></Grid>
                    <Grid size={4}><TextField label="Social" type="number" value={formData.ratings.social} onChange={(e) => updateRating('social', e.target.value)} /></Grid>
                    <Grid size={4}><TextField label="General" type="number" value={formData.ratings.general} onChange={(e) => updateRating('general', e.target.value)} /></Grid>
                </Grid>
                ...
                <Grid container spacing={2}>
                    <Grid size={4}><TextField label="Soak" type="number" value={formData.derived.soak} onChange={(e) => setFormData({...formData, derived: {...formData.derived, soak: parseInt(e.target.value)}})} /></Grid>
                    <Grid size={4}><TextField label="Wounds" type="number" value={formData.derived.wounds} onChange={(e) => setFormData({...formData, derived: {...formData.derived, wounds: parseInt(e.target.value)}})} /></Grid>
                    {formData.type === 'NEMESIS' && (
                        <Grid size={4}><TextField label="Strain" type="number" value={formData.derived.strain} onChange={(e) => setFormData({...formData, derived: {...formData.derived, strain: parseInt(e.target.value)}})} /></Grid>
                    )}
                </Grid>
