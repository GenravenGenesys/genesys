...
public class ActorHandler {
        ...
        public Mono<ServerResponse> updatePlayerCharacteristic(final ServerRequest serverRequest) {
                final String id = serverRequest.pathVariable(ID);
                String typeParam = serverRequest.pathVariable("type");
                Optional<Characteristic.Type> typeOpt = Arrays.stream(Characteristic.Type.values())
                                .filter(t -> t.name().equalsIgnoreCase(typeParam))
                                .findFirst();

                if (typeOpt.isEmpty()) {
                        return ServerResponse.badRequest().bodyValue("Invalid type: " + typeParam);
                }

                Characteristic.Type type = typeOpt.get();
                return playerService.updatePlayerCharacteristic(id, null)
